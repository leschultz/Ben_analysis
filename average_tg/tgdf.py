...
# The names of the files containing Tg
energyfile = 'tg_e.txt'
volumefile = 'tg_v.txt'
...
def fileload(dictionary, keys, datafile, path, filelist):
    ...
    outputs:
        dictionary = The dictonary with Tg
    '''
...
for path in paths:
    ...
    if datadirname not in path[0]:
        continue

    try:
        names = path[0].split('/')[-5:]
        system, composition, hold, job = names[:-1]
